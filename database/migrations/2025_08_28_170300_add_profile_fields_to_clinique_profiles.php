...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('clinique_profiles', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('clinique_profiles', 'horaire_start')) {
                $table->string('horaire_start')->nullable();
            }
            if (!Schema::hasColumn('clinique_profiles', 'horaire_end')) {
                $table->string('horaire_end')->nullable();
            }
            if (!Schema::hasColumn('clinique_profiles', 'presentation')) {
                $table->text('presentation')->nullable();
            }
            if (!Schema::hasColumn('clinique_profiles', 'additional_info')) {
                $table->text('additional_info')->nullable();
            }
            if (!Schema::hasColumn('clinique_profiles', 'moyens_paiement')) {
                $table->json('moyens_paiement')->nullable();
            }
            if (!Schema::hasColumn('clinique_profiles', 'moyens_transport')) {
                $table->json('moyens_transport')->nullable();
            }
            if (!Schema::hasColumn('clinique_profiles', 'informations_pratiques')) {
                $table->text('informations_pratiques')->nullable();
            }
            if (!Schema::hasColumn('clinique_profiles', 'jours_disponibles')) {
